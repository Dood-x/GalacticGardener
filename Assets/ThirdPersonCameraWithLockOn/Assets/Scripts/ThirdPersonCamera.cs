...
namespace ThirdPersonCameraWithLockOn{
    ...
    public class ThirdPersonCamera : MonoBehaviour {
        ...
        public bool EnableLockOn
        {
            ...
            private set { enableLockOn = value; }
        }
        [SerializeField]
        ...
        public bool EnableCamReset
        {
            ...
            private set { enableCamReset = value; }
        }
        [SerializeField]
        ...
        public bool EnableChangeTarget
        {
            ...
            private set { enableChangeTarget = value; }
        }
        ...
        [SerializeField]
        [Tooltip("Layers that are obstacles to the camera")]
        private float systemSlerp = 10f;
        public float SystemSlerp
        {
            get { return systemSlerp; }
            set { systemSlerp = value; }
        }
        ...
        //rotation of the entire camera and follow system (allows the camera to be upside down or on spherical surfaces)
        ...
        void Start() {
            ...
            Rigidbody followrb = follow.GetComponent<Rigidbody>();

            //lookDir = followrb.transform.forward;
            lookDir = followrb.transform.forward;
            ...
            smoothLookAt = followrb.position + lookoffset;


            rayHitComparer = new RayHitComparer();


            x = Vector3.Angle(followrb.transform.forward, Vector3.forward);
            if (followrb.transform.forward.x < 0) { x = 360 - x; }
            y = defaultYAngle;
            ...
            BoxCastWallCollision(characterOffset, ref targetPosition);

            this.transform.position = targetPosition;

            transform.LookAt(lookAt);
            systemRotation = Quaternion.identity;
            cameraUp = Vector3.up;
        }
        ...
        void Update () {
			...
			// check if lockon target is out of range
				if (enableLockOn && breakLockOnWhenOutOfRange && lockOnDistanceLimit > 0)
				{
		...
		void LateUpdate(){
			...
			if (camstate == CamStates.Off) {
				lookAt = followrb.position + lookoffset;
			...
			characterOffset = followrb.position + new Vector3 (0f, lookoffset.y, 0f);
            if (followrb.GetComponent<Rigidbody>())
                lookAt = followrb.position + systemRotation * lookoffset;
            else
                lookAt = followrb.position + lookoffset;
			...
			case CamStates.ThirdPersonCam:
                ...
                //cameraUp = Vector3.Lerp(cameraUp, followrb.transform.up, systemSlerp * Time.deltaTime);

                transform.LookAt (smoothLookAt, cameraUp);
			...
			case CamStates.LockOn:
				...
				if (lockOnTarget != null){
					...
					if (targetDistance > lockOnCameraFullRotationMaxDistance){
                        ...
                        //float followScalarProjection = cameraToLockOnLookAt.magnitude > cameraTofollowrb.magnitude ? cameraTofollowrb.magnitude * alpha : cameraToLockOnLookAt.magnitude * alpha;
						...
						if(debugOn && debugCameraLockOnCalculations)
						{
                            ...
                            //Debug.DrawLine(transform.position, followrb.position + lookoffset, Color.blue);
					...
					Vector3 camFreePos = characterOffset + (Quaternion.AngleAxis(defaultYAngle,Quaternion.AngleAxis(90, followrb.transform.up) * charToTargetProject) * (- charToTargetProject.normalized * distance));
					...
					transform.LookAt (smoothLookAt, followrb.transform.up);
        ...
        void FixedUpdate()
        {
            systemRotation = Quaternion.Slerp(systemRotation, follow.GetComponent<Rigidbody>().transform.rotation, systemSlerp * Time.fixedDeltaTime);
            cameraUp = Vector3.Lerp(cameraUp, follow.GetComponent<Rigidbody>().transform.up, systemSlerp * Time.fixedDeltaTime);
        }
		...
		private void MoveCamera(float x, float y, float distance, Vector3 lookAt, bool collisionYlock = false){
            ...
            //rotate by folows rotation
            if (follow.GetComponent<Rigidbody>())
            {
                //systemRotation = Quaternion.Slerp(systemRotation, followrb.GetComponent<Rigidbody>().transform.rotation, systemSlerp * Time.deltaTime);
                position = systemRotation * position;
            }
